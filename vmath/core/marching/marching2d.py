from core import gutils
from core.mutils import numerical_precision
...
def circles(x_: float, y_: float) -> float:
    """
    ...
    #                         (y_ - pxpy[1]) * (y_ - pxpy[1])) for pxpy, r in _circles)) / len(_circles)


def field_view(x_: np.ndarray, y_: np.ndarray, field_func: Callable[[float, float], float] = circles) -> np.ndarray:
...
def sort_sections_to_x_y(sects: List[Section]) -> List[Tuple[List[float], List[float]]]:
    """
