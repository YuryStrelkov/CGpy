...
from cgeo.tris_mesh.vertex import Vertex
from cgeo.vectors import Vec3, Vec2
from cgeo.camera import Camera
...
class Triangle:
    ...
    def to_screen_space(self, scr_size: Vec2) -> None:
        self.__p1 = Vec3(round(mutils.clamp(round(scr_size.x * ( self.__p1.x * 0.5 + 0.5)), 0, scr_size.x - 1)),
                         round(mutils.clamp(round(scr_size.y * (-self.__p1.y * 0.5 + 0.5)), 0, scr_size.y - 1)),
                         self.__p1.z)
        self.__p2 = Vec3(round(mutils.clamp(round(scr_size.x * ( self.__p2.x * 0.5 + 0.5)), 0, scr_size.x - 1)),
                         round(mutils.clamp(round(scr_size.y * (-self.__p2.y * 0.5 + 0.5)), 0, scr_size.y - 1)),
                         self.__p2.z)
        self.__p3 = Vec3(round(mutils.clamp(round(scr_size.x * ( self.__p3.x * 0.5 + 0.5)), 0, scr_size.x - 1)),
                         round(mutils.clamp(round(scr_size.y * (-self.__p3.y * 0.5 + 0.5)), 0, scr_size.y - 1)),
                         self.__p3.z)
